...
#endif
    setObjectName("navcoin");
    setStyleSheet("#navcoin { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #dedede, stop: 1 #efefef);  }");
    // Accept D&D of URIs
    setAcceptDrops(true);
...
void BitcoinGUI::createActions()
{
    ...
    overviewAction = new QAction(QIcon(":/icons/dashboard-white"), tr("&Dashboard"), this);
    overviewAction->setToolTip(tr("Show general overview of wallet"));
    overviewAction->setCheckable(true);
    overviewAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_1));
    tabGroup->addAction(overviewAction);

    receiveCoinsAction = new QAction(QIcon(":/icons/receive-white"), tr("&Receive"), this);
    receiveCoinsAction->setToolTip(tr("Show the list of addresses for receiving payments"));
    receiveCoinsAction->setCheckable(true);
    receiveCoinsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_2));
    tabGroup->addAction(receiveCoinsAction);

    sendCoinsAction = new QAction(QIcon(":/icons/send-white"), tr("&Send"), this);
    sendCoinsAction->setToolTip(tr("Send coins to a NavCoin address"));
    sendCoinsAction->setCheckable(true);
    sendCoinsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_3));
    tabGroup->addAction(sendCoinsAction);

    historyAction = new QAction(QIcon(":/icons/list-white"), tr("&Transactions"), this);
    historyAction->setToolTip(tr("Browse transaction history"));
    historyAction->setCheckable(true);
    historyAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_4));
    tabGroup->addAction(historyAction);

    addressBookAction = new QAction(QIcon(":/icons/user-white"), tr("&Addresses"), this);
    ...
    blockAction = new QAction(QIcon(":/icons/cube-white"), tr("&Block Explorer"), this);
    blockAction->setToolTip(tr("Explore the BlockChain"));
    blockAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    blockAction->setCheckable(true);
    tabGroup->addAction(blockAction);

    TradingAction = new QAction(QIcon(":/icons/chart-white"), tr("&Bittrex"), this);
    ...
    aboutAction = new QAction(tr("&About NavCoin"), this);
    aboutAction->setToolTip(tr("Show information about NavCoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for NavCoin"));
...
void BitcoinGUI::createToolBars()
{
    ...
    toolbar->setStyleSheet(
            "QToolButton { color: #FFFFFF; font-weight:bold; margin: 1px 0; padding: 5px 0; background-color: #43b5eb; border: 1px solid #1381b5; border-left: none; } "
            "QToolButton:hover { background-color: #71c7f0; border: 1px solid #43b5eb; border-left: none; } "
            "QToolButton:checked { background-color: #997cc5; border: 1px solid #7d59b5; border-left: none; } "
            "QToolButton:pressed { background-color: #997cc5; border: 1px solid #7d59b5; border-left: none; } "
    ...
    header->setStyleSheet("padding: 0 10px;");
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("NavCoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("NavCoin client"));
...
void BitcoinGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
    ...
    if(!clientModel->inInitialBlockDownload())
    {
        ...
        QString address = ttm->index(start, TransactionTableModel::ToAddress, parent)
                        .data().toString();
		QString dzeel = ttm->index(start, TransactionTableModel::DZeel, parent)
                        .data().toString();
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid NavCoin address or malformed URI parameters."));
    }
